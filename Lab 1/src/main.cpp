...
//individual getters for get day, month year. shorts for each so you get just the two types
...
int main()
{
	...
	ssuds::Manager M("../accounts.txt", "../transactions.txt");
	bool done = false;
	int menu_choice;
	bool show_menu = true;
	std::string response;
	...
	while (!done)
	{
		...
		case 9:
			...
			break;
		default:
			std::cout << std::endl << "INVALID menu choice: '" << response << "'" << std::endl << std::endl;
			show_menu = true;
			break;
		}
	}
