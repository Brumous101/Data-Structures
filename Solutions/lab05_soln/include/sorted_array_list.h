	...
	public:
		...
		SortedArrayList& operator = (const SortedArrayList & other)
		{
			...
			if (this != &other)
			{
				...
				delete[] ArrayList<T>::mData;
				ArrayList<T>::mCapacity = other.mCapacity;
				ArrayList<T>::mData = new T[ArrayList<T>::mCapacity];
				for (unsigned int i = 0; i < other.mSize; i++)
					ArrayList<T>::mData[i] = other.mData[i];
				ArrayList<T>::mSize = other.mSize;
			}
		...
		int find(const T & val, int start_index = 0) const
		{
			int left, right;
			left = start_index;
			right = ArrayList<T>::mSize - 1;
			while (left <= right)
			{
				int mid = (left + right) / 2;
				if (val == ArrayList<T>::mData[mid])
					return mid;
				else if (val < ArrayList<T>::mData[mid])
					right = mid - 1;
			}
			return -1;
		}
