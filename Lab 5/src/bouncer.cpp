...
sf::Vector2f ssuds::Bouncer::getVelocity()
{
	return mVelocity;
}

sf::Vector2f ssuds::Bouncer::getPosition()
{
	return mPos;
}

std::ostream& ssuds::operator<< (std::ostream& os, Bouncer& bouncerObject)
{
	float currentBouncerVelocity;
	currentBouncerVelocity = sqrtf((bouncerObject.mVelocity.x * bouncerObject.mVelocity.x) + (bouncerObject.mVelocity.y * bouncerObject.mVelocity.y));
	os << "<Bouncer:";
	os << "pos=[";
	os << bouncerObject.getPosition().x << ", " << bouncerObject.getPosition().y << "], ";
	os << "vel=[";
	os << bouncerObject.getVelocity().x << ", " << bouncerObject.getVelocity().y << "], ";
	os << "speed=";
	os << currentBouncerVelocity << ">";
